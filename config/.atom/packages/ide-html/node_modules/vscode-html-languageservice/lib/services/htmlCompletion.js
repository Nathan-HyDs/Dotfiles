(function (factory) {
    ...
    function doComplete(document, position, htmlDocument, settings) {
        ...
        function collectCharacterEntityProposals() {
            ...
            if (k >= 0 && text[k] === '&') {
                var range = vscode_languageserver_types_1.Range.create(vscode_languageserver_types_1.Position.create(position.line, characterStart), position);
                for (var entity in htmlEntities_1.entities) {
                    if (strings_1.endsWith(entity, ';')) {
                        result.items.push({
                            ...
                            textEdit: vscode_languageserver_types_1.TextEdit.replace(range, entity),
                            insertTextFormat: vscode_languageserver_types_1.InsertTextFormat.PlainText
                        });
                    }
                }
